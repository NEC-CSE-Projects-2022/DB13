...
app = Flask(__name__, template_folder='../../frontend/src', static_folder='../../frontend/public')
...
# Load Kosmos-2 weights (online)
try:
    # Removed local load, using pretrained model from Hugging Face
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    kosmos_model.to(device)
    print(f"✅ Kosmos-2 model loaded successfully to {device}!")
except Exception as e:
    print(f"Error loading Kosmos-2 model: {e}")
    # Do not exit, allow other models to work if this fails (though it shouldn't if internet is available)
...
try:
    ...
    print(f"⚠️ Could not load GPT-2 model: {e}")
...
try:
    ...
    sbert_model.eval()
    print("✅ SentenceBERT loaded successfully!")
except Exception as e:
    print(f"⚠️ Could not load SentenceBERT: {e}")
...
def generate_clip_ranking(image_bytes, captions):
    if clip_model is None or clip_processor is None:
        return captions[0] if captions else "No caption available"
    try:
        image = Image.open(io.BytesIO(image_bytes)).convert("RGB")
        ...
        print(f"Error in CLIP ranking: {e}")
        return captions[0]
...
def generate_gpt2_enhancement(caption):
    ...
    try:
        ...
        print(f"Error in GPT-2 enhancement: {e}")
        return caption
...
def generate_sbert_fusion(captions):
    ...
    try:
        ...
        print(f"Error in SBERT fusion: {e}")
        return captions[0]
